...
function defaultOutputPath({ compressed, embedded }) {
  if (embedded && compressed) {
    return "dist/character-sample-embedded-compressed.fbx";
  }
  if (embedded) {
    return "dist/character-sample-embedded.fbx";
  }
  return compressed ? "dist/character-sample-compressed.fbx" : "dist/character-sample.fbx";
}
...
await writeFile(outputPath, exportFbx(createSkinnedMorphScene(), exportOptions({ compressed, embedded })));
